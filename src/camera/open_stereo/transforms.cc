...
std::unordered_map<std::string, cv::Mat> RightTopPad::operator()(
  ...
  for (auto& item : sample) {
    ...
    } else if (key == "disp" || key == "disp_right" || key == "occ_mask" ||
               key == "occ_mask_right") {
...
cv::Mat RightTopPad::PadImage(const cv::Mat& image,
                              int target_height,
                              int target_width,
                              int borderType,
  ...
  cv::Mat padded_image;
  cv::copyMakeBorder(
      image, padded_image, pad_top, pad_bottom, pad_left, pad_right, borderType, value);
  return padded_image;
}

TransposeImage::TransposeImage() {}

std::unordered_map<std::string, cv::Mat> TransposeImage::operator()(
    std::unordered_map<std::string, cv::Mat>& sample) const {
  for (auto& item : sample) {
...
std::unordered_map<std::string, cv::Mat> NormalizeImage::operator()(
    std::unordered_map<std::string, cv::Mat>& sample) const {
  for (auto& item : sample) {
...
CropOrPad::CropOrPad(int target_height, int target_width)
    : target_height_(target_height),
      ...
      crop_fn_(target_height, target_width),
      pad_fn_(target_height, target_width) {}

std::unordered_map<std::string, cv::Mat> CropOrPad::operator()(
    std::unordered_map<std::string, cv::Mat>& sample) const {
...
DivisiblePad::DivisiblePad(int by, const std::string& mode) : by_(by), mode_(mode) {}

std::unordered_map<std::string, cv::Mat> DivisiblePad::operator()(
    std::unordered_map<std::string, cv::Mat>& sample) const {
  int h = sample.at("left").rows;
  int w = sample.at("left").cols;
  ...
  for (auto& item : sample) {
    if (item.first == "left" || item.first == "right") {
      cv::copyMakeBorder(
          item.second, item.second, pad_top, pad_bottom, pad_left, pad_right, cv::BORDER_REPLICATE);
    } else if (item.first == "disp" || item.first == "disp_right" || item.first == "occ_mask" ||
               item.first == "occ_mask_right") {
      cv::copyMakeBorder(item.second,
                         item.second,
                         pad_top,
                         pad_bottom,
                         pad_left,
...
std::unordered_map<std::string, cv::Mat> Transform::operator()(
    std::unordered_map<std::string, cv::Mat>& sample) const {
