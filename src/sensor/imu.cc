...
void IMU::ReadData(const std::string& file_path) {
  if (file_path.empty()) {
    ...
    if (file.is_open()) {
        ...
        while (getline(file, line)) {
            ...
            if (iss >> timestamp >> gx >> gy >> gz >> ax >> ay >> az ) {
                ...
                data_vec_.emplace_back(IMUData(timestamp / 1.0e9, acc, gyr));
            }
            if (count % 10000 == 0) {
        ...
        file.close();
        logger_.Log(INFO) << "Finished reading total of " << data_vec_.size() << " data entries from IMU sensor.";
    } else {
...
std::vector<IMUData> IMU::GetVecData() const {
    return data_vec_;
}
