...
DEFINE_string(img1, "../include/camera/config/1.png", "第一张测试图像");
DEFINE_string(img1_depth, "../include/camera/config/1_depth.png", "第一张测试图像的深度图");
DEFINE_string(img2, "../include/camera/config/2.png", "第二张测试图像");
...
TEST_F(SFMTest, P2PTest) {
  ...
  Eigen::Matrix3d essential_mat = Eigen::Matrix3d::Identity();
  std::vector<bool> inliers;
  if (sfm_->FindEssentialMatrix(keypoinyts1, keypoinyts2, good_matches, &essential_mat, &inliers) ) {
