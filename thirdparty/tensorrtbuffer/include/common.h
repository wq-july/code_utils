...
#define CHECK_RETURN_W_MSG(status, val, errMsg)                                                 \
  do {                                                                                          \
    if (!(status)) {                                                                            \
      tensorrt_log::gLogError << errMsg << " Error in " << __FILE__ << ", function " << FN_NAME \
...
inline std::string locateFile(const std::string& filepathSuffix,
  ...
  if (filepath.empty()) {
    const std::string dirList = std::accumulate(directories.begin() + 1,
                                                directories.end(),
                                                directories.front(),
...
inline void setDummyInt8DynamicRanges(const IBuilderConfig* c, INetworkDefinition* n) {
  ...
  if (c->getFlag(BuilderFlag::kINT8)) {
    tensorrt_log::gLogWarning << "Int8 calibrator not provided. Generating dummy per-tensor "
                                 "dynamic range. Int8 accuracy is not guaranteed."
                              << std::endl;
...
inline void enableDLA(IBuilder* builder,
  ...
  if (useDLACore >= 0) {
    ...
    // TensorRT10.1.0 不存在这个标志， 这个表示强制执行xx位的精度计算
